...
%{ voice-spec.scm
    <voice-spec>: class representing a single voice part, i.e. Tenor or Baritone
        ...
        Example instance creation: (define tenor-spec (make <voice-spec> #:name "Tenor" #:clef "treble_8"))
 %}
#(begin
...
%{ staff-spec.scm
    <staff-spec-base>: base class for staff specs
    ...
    <one-voice-staff-spec>: specification for a staff with a single voice; inherits from <staff-spec-base>
        #:voice : <voice-spec> defining voice for this staff
            getter method: (staff-spec-voice <one-voice-staff-spec>)
    
    <two-voice-staff-spec>: specification for a staff with two voices; inherits from <staff-spec-base>
    ...
    (staff-spec-all-voice-names STAFF_SPEC): utility method returning a list of all voice names on the given staff

    (staff-spec-all-prefixes STAFF_SPEC): utility method returning a list containing the staff name followed by all voice names on the given staff
    ...
    (generate-staff-definition STAFF_SPEC): uses the staff spec to generate a LilyPond staff, using the music functions defined in the built-in LilyPond vocal-tkit.ly
...
%{ score-spec.scm
    staves: global list defining the ordered set of staves to include in the score, from top to bottom. Do not modify directly; use (set-staves!)

    (set-staves! STAVES_LIST): sets the global staff list to be the given list of staves

    (staves-voice-prefixes): returns the names of all staves in the global list and the voices they contain, for defining prefixes as in the LilyPond builtin base-tkit.ily
