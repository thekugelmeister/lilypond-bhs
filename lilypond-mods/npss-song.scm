...
;;;; Updated by: Jeremy Marcus <github.com/thekugelmeister>
;;;; Changes: Added support for ties, treating them like melismata.
;;;;
...
(define-public *base-octave-shift* (make-parameter 0))
...
;; JEREMY: Modified to recognize ties.
       ...
       (cond
        ...
        ((or (music-name? music 'EventChord)
         ...
         (let ((note (find-child-named music 'NoteEvent))
           ...
           (cond
            (note
             ;; JEREMY: Tie modifications in this branch.
             (debug "Note" note) 
             (let* ((pitch (ly:music-property note 'pitch))
                    ...
                    (tie-events (filter identity (list
                                                  (find-child-named music (quote TieEvent)))))
               ...
               (let* ((is-joined (if (not (null? tie-events))
                                     1
                                     in-slur))
                      (note-spec (make-note #:pitch pitch #:duration duration #:joined is-joined
...
;;; Output
(define (pitch->eqtemphz pitch)
  ;; Generates frequency (Hz) in equal temperament from the given pitch.
  ;; Formula from http://www.ericweisstein.com/encyclopedias/music/EqualTemperament.html
  ;; 
  ;; ly:pitch-semitones is in terms of semitones away from middle c; formula expects
  ;; semitones away from A4 (which is 9 semitones away from middle c).
...
(define (write-lyrics-element port text slur-list)
  (let ((fmt "~{~{~a~^+~}~^,~}")
    ...
    (format port "<DURATION BEATS=\"~@?\"><PITCH NOTE=\"~@?\">~a</PITCH></DURATION>~%"
            ...
            fmt (transform (compose pitch->eqtemphz note-pitch))
            text)))
